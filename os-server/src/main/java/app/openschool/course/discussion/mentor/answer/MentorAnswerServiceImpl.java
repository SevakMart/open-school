...
public class MentorAnswerServiceImpl implements AnswerService {
  ...
  public MentorAnswer create(
    ...
    MentorQuestion extractedMentorQuestion =
            mentorQuestionRepository.findById(requestDto.getQuestionId()).orElseThrow(IllegalAccessError::new);

    checkingDataConsistency(extractedMentorQuestion,extractedEnrolledCourse,email, requestDto.getQuestionId());

    return mentorAnswerRepository.save(prepareAnswer(extractedEnrolledCourse,requestDto,extractedMentorQuestion));
  }

  @Override
  public MentorAnswer update(UpdateAnswerRequest request,
                             Long answerId,
                             Long questionId,
    ...
    MentorAnswer mentorAnswer =
            mentorAnswerRepository
                    .findMentorAnswerByIdAndUserEmailAndQuestionId(answerId,questionId, enrolledCourseId, currentUserEmail)
