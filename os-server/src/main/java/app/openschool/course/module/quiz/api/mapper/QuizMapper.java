...
public final class QuizMapper {
  ...
  public static Page<QuizDto> toQuizDtoPage(Page<Quiz> quizzesPage) {

    List<QuizDto> collect =
        quizzesPage.toList().stream().map(QuizMapper::quizToQuizDto).collect(Collectors.toList());
    return new PageImpl<>(collect, quizzesPage.getPageable(), quizzesPage.getTotalElements());
  }
